...
键盘机型在编写界面内置**五笔86**输入法，可直接用中文编写消息。消息框与按键图例之间的横条上有 **中/EN** 切换按钮、当前编码与候选字。

- **中/EN 切换** — 点击按钮，或按 Cardputer 的 **Fn+Space** / T-Lora Pager 的 **Sym+Space**。模式跨编写会话保持；编码过程中方向左右键（或 `,`/`.`）翻页。
- **输入** — 字母逐键构成五笔编码（最多 4 键，a–y；Z 不是编码键，按下会直接输入字母 z）。横条显示 `g: 1.一 2.…`，每页 5 个候选（Cardputer 窄屏每页 3 个）。一级/二级简码内置在码表中：一级简码首字排第一（如 `g` 首选"一"），两键即可取到二级简码字；同一字的简码/全码重复候选会自动去重。
- **上屏** — 数字 **1–5** 选择对应候选，**空格**上屏首选，**回车**将原始字母直接上屏。
- **回退** — **退格**删除一个编码字母（删空后恢复删除消息文字），**Esc** 清空编码。

引擎（[src/wubi_ime.cpp](src/wubi_ime.cpp)）对固件内码表做前缀二分查找。码表由 `tools/gen_wubi_ime.py` 从 [rime/rime-wubi](https://github.com/rime/rime-wubi) 的 `wubi86.dict.yaml`（LGPL-3.0）生成：10,739 个编码 / 7,000 个高频汉字（约 111KB），候选已按 CJK 回退字库的实际覆盖过滤（取两款机型共同覆盖的字频前 7,000 字），不会出现缺字方框。重新生成：

```bash
python3 tools/gen_wubi_ime.py path/to/wubi86.dict.yaml
```
